...
def fit_genetic_algorithm(M_PROBABILITY, X_PROBABILITY, P_SIZE, N_GEN, SEL_M, CROSS_M, MUT_METHOD, K, items, encoding,
    ...
    for user in tqdm.tqdm(range(items.shape[0])):
        ...
        for gen in tqdm.tqdm(range(N_GEN)):
            ...
            universe_evaluation = cupy.fromiter(
                (evaluate_chromosome(chromosomes[i], optim_values) for i in range(chromosomes.shape[0])), cupy.int64)
            # stop if 90% accuracy
            ...
            if gen % 1000 == 999:
                print(cupy.mean(universe_evaluation).astype(cupy.int64))

...
def fetch_neighborhood(user, tabular_np, k):
    ...
    for i in range(data_vector.shape[0]):
        if i != user:
            ...
            if cupy.equal(cupy.where(user_vector == 0)[0].shape, cupy.where(data_vector[i][0] == 0)[0].shape):
                if cupy.equal(cupy.where(user_vector == 0)[0], cupy.where(data_vector[i][0] == 0)[0]):
...
def roulette_wheel_selection(population, optim):
    ...
    pearsons = cupy.fromiter((evaluate_chromosome(population[i], optim) for i in range(population.shape[0])),
                              pearsons.dtype) + 1
...
def genetic_algorithm_convergence(evaluation_best_chr, evaluation_overall_gen, gen, population, optim,
    ...
    for conv_method in ("best_chr", "percentage", "generation"):
        if conv_method == 'best_chr':
            ...
            if cupy.abs(evaluation_overall_gen[gen] - evaluation_overall_gen[gen - 1]) < PERCENT_CHR_CONV_LIM:
                stop = True
    ...
    for user in tqdm.tqdm_notebook(range(items.shape[0])):
        ...
        for gen in tqdm.tqdm_notebook(range(N_GEN)):
            ...
            if cupy.mean(cupy.fromiter((evaluate_chromosome(chromosomes[i], optim_values)
                ...
                print("Mean accuracy: ", cupy.mean(cupy.fromiter((evaluate_chromosome(chromosomes[i], optim_values)
            ...
            U_EVALS_LIST.append(cupy.mean(cupy.fromiter((evaluate_chromosome(chromosomes[i], optim_values) for i
                                                           in range(chromosomes.shape[0])), cupy.int64)))
        # save population
        save_population(user, chromosomes, optim_values)
        # save different fitness metrics
        save_evaluations(user, chromosomes, optim_values)
...
def main():
    ...
    M_PROBABILITY, X_PROBABILITY, P_SIZE, N_GEN, SEL_M, CROSS_M, MUT_METHOD, K, ENC, BEST_CHR_CONV_LIM, \
